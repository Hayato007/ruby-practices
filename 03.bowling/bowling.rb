...
#          frame.sum + frames[n + 1].sum
#         else  #１フレームと2フレーム目がストライクの場合、３フレーム目の情報が必要
#           frame.sum + frames[n + 1].sum + frames[n + 2].sum # ストライクの場合、次のフレームの投球を取得しなければいけないが,２フレーム先まで取得してしまっているため、点数に差異が生じる。
#         end
#       end
#     elsif frame.sum == 10 # スペアの場合
#       frame.sum + frames[n + 1][0]
...
#     if frame[0] == 10 # ストライクの場合
#       if frames[n + 1] && frames[n + 2] ## 9フレーム目と10フレームがXの場合9フレームは11フレームを探してしまうので、それを抑制するコードである。
#         # if frame[n + 1] =! 10 # ２フレーム目がストライクでない場合のコードを追加しなくてはいけない！！！！！！！！！
#         #                 # ストライク一回だけの場合は次フレームの合計のみを取得すれば良い
#         # else                #１フレームと2フレーム目がストライクの場合、３フレーム目の情報が必要
#         frame.sum + frames[n + 1].sum + frames[n + 2].sum
#         # ストライクの場合、次のフレームの投球を取得しなければいけないが、
#         # ２フレーム先まで取得してしまっているため、点数に差異が生じる。
#         # end
#       else
#         frame.sum
#       end
